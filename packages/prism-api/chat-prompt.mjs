/** Shared chat system prompt for hosted API */
...
export function buildChatSystemMessage({ mode = 'ask', context = '', projectPath = '' } = {}) {
  const modeKey = MODE_RULES[mode] ? mode : 'ask';
  const parts = [BASE_RULES, MODE_RULES[modeKey]];

  if (projectPath?.trim()) {
    parts.push(`Open project folder: ${projectPath.trim()}`);
  }
